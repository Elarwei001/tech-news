...
今早的大新闻：[Astral 加入 OpenAI](https://astral.sh/blog/openai)（Astral 博客公告）以及 [OpenAI 收购 Astral](https://openai.com/index/openai-to-acquire-astral/)（OpenAI 公告）。Astral 是 [uv](https://simonwillison.net/tags/uv/)、[ruff](https://simonwillison.net/tags/ruff/) 和 [ty](https://simonwillison.net/tags/ty/) 背后的公司——这三个开源项目在 Python 生态中承担着越来越重要的角色。我有些想法！
...
这笔交易最糟糕的走向是：OpenAI 利用对 uv 的所有权，作为与 Anthropic 竞争的筹码。
...
> 不过，在看过代码和 uv 的实现之后，即便是最坏的情况，这也是一个**非常易于 fork 和维护的项目**。我相信即使 Astral 关闭或做出任何令人难以置信的授权许可方面的骚操作，社区的处境也会比 uv 存在之前更好。
